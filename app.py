...
def main():
    ...
    if page == "📊 Water-Table Map":
        ...
        st.header("Forecasting Average Groundwater Table Using Random Forest")
        levels = load_levels()
        well_cols = [c for c in levels.columns if c.upper().startswith("W")]
        levels["Year"] = levels["Date"].dt.year
